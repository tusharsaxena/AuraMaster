...
| `modules/Container.lua` | One live container: its anchor, handle and unlocked placeholder outline, shown only while predicted empty (a plain frame, `Style.DrawEdge` strips; `ApplyHang`, `PredictEmpty`), its optional name label (`ApplyLabel`, placed by `Anchors.PlaceLabel`), building, updating or retiring its engine, restyling, the per-apply class snapshot, the show ladder; bucket `applyContainer` |
...
| `settings/Layout.lua` | The Layout page, a sub-page of Containers (`N-2`, D6): frame, anchor (Screen / Another container / Named frame / Offset, drawn by attach mode), growth, mouse, the name label (show, X/Y and its font, batch 8 NL-4); Pick a frame |
...
| `test_anchors_edges.lua` | `modules/Anchors.lua` and `settings/Layout.lua`: the side a follower sits on (batch 9 AP-1..AP-4): the nine tokens and their points for every growth, `after-start` pinned to the old derived points, which sides are allowed, the runtime fallback, the seam on a side, the default side of a new attachment, the Side row's refusal, and the parents re-applied |
