...
| `test_anchors_steady.lua` | `modules/Anchors.lua` (batch 11 T9): a center or end join on a parent one element across lands in the same place with the parent's engine empty (a 1x1 rect at its start corner) as populated, and on its anchor (slot); an end join and growth left likewise; a side join steady on y under a one-row parent; a parent several elements across not rewritten; the offset converted between scales; start-aligned pairs as before |
