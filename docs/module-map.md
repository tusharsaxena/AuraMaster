...
| `modules/Style_Icons.lua` | Builds and dresses an icon button: aspect-correct crop, cooldown swipe, the dispel border on its own frame above ours (`dispelHost`), tinted from the profile's palette; preview fill |
