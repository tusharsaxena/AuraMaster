...
| `modules/Anchors.lua` | Places a container's anchor on the screen, another container or a named frame; cycle check; the flow a container attached to another inherits (`FlowRoot`, `EffectiveLayout`, `Followers`) and what an attachment would change about it (`FlowChangeOnAttach`, batch 9 GC-1); the side a follower sits on, stored relative to the flow (`EDGES`, `ParseEdge`, `EdgePoints`, `EdgeAllowed`, `ResolvedEdge` with its runtime fallback, `DefaultEdge` for a new attachment, `EdgeLabel`, and `DerivedPoints`, the default side); what those attached to a container hang from, its preview extent, its one-element anchor (only while predicted empty) or its engine (`HangMode`, from the instance's `hangMode`, which `ContainerClass:ApplyHang` records, and the room left for its strip, from `stripShown`), re-placed when that changes (`PlaceAttached`); the seam gap to an attached container, the child's own spacing along the chain or its gap across on a side, with the X/Y offsets as a nudge on top (`SeamOffset`); which side of its first element the strip sits on (`StripSide`, batch 9 SEP-3: before for a root or a follower on a side, else behind, ahead or inside, whichever no direct follower holds) and where the strip and the optional name label sit (`StripPoints`, `PlaceLabel`, the strip pushed out past a shown label, an ahead follower's past its parent's rows); the join pin, a gold diamond at the child's attach point while unlocked (SEP-2), and the join the strip's tooltip names (`JoinText`) and the label's justify in effect (`LabelJustify`, a pick or the style default); pending frame re-resolve; the drag handle (LibKa0s-Widgets-1.0's `DragHandle`, handed this addon's strings, `Style.DrawEdge` painter, `NS.Secrets.NumberOr` guard and callbacks, among them the close mark's `disableContainer`, which turns the container off through `NS.SetByPath`), `HANDLE_LEVEL` above its anchor and above an attached target's |
...
| `settings/OptionsSetup.lua` | The `LibKa0s-Options-1.0` descriptor (its `get` shows a row's `panelGet`; its `set` hands a row's `confirmWrite` popup the write instead, `confirmFirst`) and its load-completing stub; the container banner (`ContainerBanner`, with the Containers page's New container as the library `PageBanner`'s `action`); `RenderPage`, which maps a page spec (its own tabs filtered by aura type, `intro`, `disabledFor` and the muted-red `disabledNotice`, `afterGroup`, `pairWith`) onto the library's `O.RenderTabbedSchema`, and `RenderContainerPage`; `NS.RegisterContainerPage`, `NS.OpenOptionsPage`, `NS.RequestPanelRefresh` |
...
| `test_anchors_seam.lua` | `modules/Anchors.lua`: the seam to an attached container (SS-1..SS-3): the child's own spacing or line spacing in the direction the chain stacks, the X/Y nudge on top, a chain growing up, frame and screen containers untouched, the preview seam, and the child's strip beside its first element |
| `test_anchors_edges.lua` | `modules/Anchors.lua` and `settings/Layout.lua`: the side a follower sits on (batch 9 AP-1..AP-4) and what an attachment changes about its flow (`FlowChangeOnAttach`, GC-1): the nine tokens and their points for every growth, `after-start` pinned to the old derived points, which sides are allowed, the runtime fallback, the seam on a side, the default side of a new attachment, the Side row's refusal, and the parents re-applied |
...
| `test_anchors_label.lua` | `modules/Anchors.lua` and `modules/Container.lua`: the name label (NL-1..NL-4): off by default, its text the container's name, placed where the strip sits locked or unlocked, the strip pushed out past it while unlocked (D6), beside the first element on a follower, its justify (a pick, or the style default of B9 E7), its font, a rename, and the Layout tab |
| `test_anchors_strip.lua` | `modules/Anchors.lua`, `modules/Container.lua` and `modules/Preview.lua` (batch 9 SEP-1..SEP-3, E4): which side of its first element a strip sits on (`StripSide`) and where (`StripPoints`, each side mirrored by the growth), an ahead follower pushed past its parent's rows, the clamp per side, the label justify that follows it, no allocation per call, the join pin and its tooltip line, and the test-mode block outline |
...
| AceConsole-3.0 | `/am` and `/auramaster` registration (`settings/Slash.lua:560-561`) |
| AceDB-3.0 | `AuraMasterDB` and its profiles (`core/Database.lua:263`) |
