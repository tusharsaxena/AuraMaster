...
| `modules/Container.lua` | One live container: its anchor, handle and unlocked outline (a plain frame, `Style.DrawEdge` strips), its optional name label (`ApplyLabel`, placed by `Anchors.PlaceLabel`), building, updating or retiring its engine, restyling, the per-apply class snapshot, the show ladder; bucket `applyContainer` |
