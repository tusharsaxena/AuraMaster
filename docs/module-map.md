...
| `modules/Diagnostics.lua` | `/am diagnostics` and `/am debug diagnostics`: the one-shot diagnostic report written to the debug console through the ungated `NS.DebugLog:Add` (header and apply queue, non-default settings, the auras on each unit, and per container its filters, plan verdict, non-default rows and shown buttons; while the addon is disabled or stood down a plain header line says so and each `[Plan] #N not built` names its reason, batch 10 F8), secret-safe and capped below the console buffer (`docs/debug.md`) |
...
| `test_anchors_strip.lua` | `modules/Anchors.lua`, `modules/Container.lua` and `modules/Preview.lua` (batch 9 SEP-1, SEP-2, E4, with SEP-3 as batch 10 leaves it): a behind follower's strip and clamp, the icons label justify, the join pin and its tooltip line, and the test-mode block outline |
| `test_anchors_column.lua` | `modules/Anchors.lua` and `modules/Container.lua` (batch 10 F1-F5): every strip and label before its own block in its own column (strip, label, block), the chain spread by a follower's own strip and label rows, locked, unlocked and in test mode, growing down and up, a chain of three, the owner's Text chain, a side follower pushed past its parent's furniture, the re-place on a visibility pass (none under lockdown, none allocated when nothing changed), and the join pin |
