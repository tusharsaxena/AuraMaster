...
| `test_anchors_label.lua` | `modules/Anchors.lua` and `modules/Container.lua`: the name label (NL-1..NL-4): off by default, its text the container's name, placed where the strip sits locked or unlocked, the strip pushed out past it while unlocked (D6), beside the first element on a follower, its font, a rename, and the Layout tab |
