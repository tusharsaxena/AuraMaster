...
| `settings/GeneralSpells.lua` | General → Spell Categories (one spell category's ID list over the profile's `categorySpells` and its restore, or — for the Weapon enchants entry — the profile-wide `enchantSlots` toggles), the *This category* / *Make a new category* block that creates, renames and deletes a player's own category and offers a way out of a record the sync cannot read, the overlap notes off `FC.ClaimingCategories`, and General → Dispel Colors (the five profile-wide `dispelColors.<type>` rows); publishes `MarkedName` (the one `(yours)` marker, read by the Filters grid too), `RestoreStarters` and `Select`; registers nothing itself, `settings/General.lua` registers its rows and draws its tabs |
