...
| LibKa0s v1.50.0 | Ten modules wired, one setup file each — table below |
...
The layout-§1 census, enforced by the kit's `tests/_kit/test_layout_cap.lua`. Line counts are from `wc -l` over `git ls-files '*.lua'`, leaving out `libs/` and `tests/_kit/`, on 2026-09-23. Each row points at the `layout-§1` deviation row above that covers it.

| File | Lines | Terminal state |
|---|---|---|
| `settings/GeneralSpells.lua` | 1528 | Deviation row above (`layout-§1`, `settings/GeneralSpells.lua`); peel seam: Dispel Colors |
| `tests/test_database.lua` | 1702 | Deviation row above (`layout-§1`, `tests/test_database.lua`); peel seam: user categories |
| `tests/test_filtercompiler.lua` | 1567 | Deviation row above (`layout-§1`, `tests/test_filtercompiler.lua`); peel seam: user categories |
| `tests/test_pages_general.lua` | 1761 | Deviation row above (`layout-§1`, `tests/test_pages_general.lua`); peel seam: Your categories |
