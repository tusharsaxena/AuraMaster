...
The layout-§1 census, enforced by the kit's `tests/_kit/test_layout_cap.lua`. Line counts are from `wc -l` over `git ls-files '*.lua'`, leaving out `libs/` and `tests/_kit/`, on 2026-09-23. Each file's terminal state is an open issue naming the seam a peel would follow, one of layout-§1's three.
