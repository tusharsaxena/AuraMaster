...
**What this addon does.** Every container is one engine (`modules/Container.lua:169`). The addon
...
- **Defers every structural apply and restyle** while `Compat.AurasAreSecret()` or
  `InCombatLockdown()` is true (`ContainerManager.MustDefer`, `modules/ContainerManager.lua:155`),
...
`AddAuraGroup` (`modules/Container.lua:173-177`). Every anchor frame, and the frame picker's outline,
...
candidate filters are compared with `FilterCompiler.Signature` first (`modules/Container.lua:248-250`). A
...
- **Visibility in combat is the engine's `SetEnabled`**, not `Show`/`Hide` on an ancestry holding
  aura buttons (`modules/Container.lua:368`).
