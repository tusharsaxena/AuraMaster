...
**What this addon does.** Every container is one engine (`modules/Container.lua:172`). The addon
...
`AddAuraGroup` (`modules/Container.lua:176-180`). Every anchor frame, and the frame picker's outline,
...
candidate filters are compared with `FilterCompiler.Signature` first (`modules/Container.lua:251-253`). A
...
- **Visibility in combat is the engine's `SetEnabled`**, not `Show`/`Hide` on an ancestry holding
  aura buttons (`modules/Container.lua:371`).
