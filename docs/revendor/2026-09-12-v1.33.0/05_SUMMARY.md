...
- `tests/test_bulklog.lua:149`: said the kit's AceDB passes the CURRENT profile to every callback; it now says revision 18 passes OnProfileCopied the source, and that the `'.-'` pattern does not pin it.
...
|---|---|---|---|
| Before the copy | 267 passed, 0 failed, 0 skipped, 267 total | 0 / 0 in 63 files | clean, no function above CCN 15 |
| After the copy, the roll and the comments | 267 passed, 0 failed, 0 skipped, 267 total | 0 / 0 in 63 files | clean, no function above CCN 15 |
