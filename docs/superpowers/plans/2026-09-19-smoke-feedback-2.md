...
**Current position (2026-09-20):** all tasks done, owner follow-ups A and B (rows 14-15) and item B2-3 (row 16) included; `feat/smoke-feedback-2` merged to `master` and pushed. The owner verified `docs/smoke-tests.md` section U (143-161) in game on 2026-09-20. Follow-up row 17 (the outline, the handle and the picker's outline never read a secret size) is on `fix/secret-safe-outline`, awaiting the owner's go-ahead to merge and push, and in-game smoke item 162.
...
| 4 | Item 4 — dispel-colored bar background / fill honor opacity | AuraMaster | done | T4 commit: dispel map entries opaque; dispel-mode region SetAlpha(opacity x color alpha), static unchanged; smoke 126 gains the in-combat alpha check (child-frame fallback not built); +3 tests (1126/1126) |
...
| 16 | Owner feedback B2-3 — aura-button borders never read a secret size: Solid drawn with strips, other styles a guarded backdrop | AuraMaster | done | B2-3 commit: Style.ApplyBorder draws Solid as four strips (SetColorTexture, sides inset so no corner doubles) on a plain border frame (Style.NewBorder, never BackdropTemplate); another style is a backdrop on a plain child frame with BackdropTemplateMixin mixed in (no OnSizeChanged script), SetBackdrop only when edge/size changed and the size reads plain, else recolor only; Icons/Bars border steps and the bar icon border through Style.GuardedBorder (report, hide, bindings still run); Border style tooltip on the four rows; mock secret sentinel + __layOut + backdrop arithmetic model; midnight-quirks, ARCHITECTURE taint note + known limitation, settings-panel, smoke 158-161; +15 tests (1157/1157) |
| 17 | Follow-up to B2-3 — the unlocked outline, the drag handle and the frame picker's outline never read a secret size | AuraMaster | done | Commit "Outline and handle never read a secret size — plain frames, strip edges" on `fix/secret-safe-outline`: the three BackdropTemplate frames are plain frames (the picker's keeps DisableUntrustedLayoutScriptsTemplate); edges drawn once with the border strips (Style.DrawEdge), the handle's fill a BACKGROUND texture (SetColorTexture 0,0,0,0.75); look unchanged (outline 1px white 0.35, handle 1px gold 0.6, picker 2px blue); tests/border_strips.lua recorderTextures; midnight-quirks, ARCHITECTURE taint note, module-map, smoke 162; Style/Container citations re-pointed; +3 tests, 1 updated (1160/1160) |
