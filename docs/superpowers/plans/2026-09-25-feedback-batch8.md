...
|---|---|---|---|---|---|---|
| P1 spark neutral ADD | SP-1 SP-2 | AM | — | done | 6eed689 | spark ADD + desaturated per dress in applySurfaces; SP-1 BLEND tests replaced; KL + check 85 rewritten |
...
| P3 dispel border shape | DB-1 DB-2 | AM | — | done | 81a818e | dispel edge = 4 white strips in the Solid shape (Style.TintEdge), PreserveAsset with no map; 1 px when border hidden/None/0; preview tints via Compat.SetAuraBorderColor (replaces SetAuraBorderAtlas) |
| P4 icon attach points | IA-1 IA-2 | AM | — | done | 565fc38 | DerivedPoints ignores the axis: child stacks TOP{H}->BOTTOM{H} (up: BOTTOM{H}->TOP{H}); DERIVED rows + label test flipped, axis-independence + icon-row Place tests; smoke 67 row case |
...
| P11 /am debug diag | DG-1…4 | AM | P10 | done | 1e3c29d + fb4ff7f | Review (B8-P11R): the predictions list now flags its 40-id cap so the truncated line appears (DG-4); CM test-seam comments name Diagnostics as a reader. modules/Diagnostics.lua (Build/Run, ungated DebugLog:Add + one L chat line; header/queue, non-default cfg via Slash.FormatValue stripped, auras per unit/filter, per container Cont/Filt/Plan verdict in sync/PENDING/DRIFT/not built/Cfg/Shown + predicted; no aura or button call while secret; caps 1200/100/40 + truncated line); runDebug diag branch first; CM.QueueSnapshot, NS.RowApplies; docs/debug.md; test_diagnostics suite; doc citations re-pointed |
| P12 docs sync + full battery | all | AM | P11 | done | 4b660aa + 788e47d | review: follower name label wording corrected (beside the first element, over none of the parent's) in known-limitations and smoke 204; test-cases regenerated (1470); counts re-verified live (schema 255: Filters 46, Layout 35, Text 37; 50 files; 22 commands/shims; 4 messages); schema.md label block + 8 whole sections; settings-panel Name label + dispel strips; module-map suite order + seam/label suites; data-flow seam/label/X/autosize; known-limitations X/label/autosize/diag; README badge + X/label/Size to fit/diag; smoke section Z (197-208 + item index, owner to run); battery green |
| P13 /am diagnostics + README | DG-1 (amended 2026-09-25) + README | AM | P12 | done | d386957 | owner request after the plan: the report runs from exactly `/am diagnostics` (new top-level verb, live while disabled) and `/am debug diagnostics`; the `diag` alias is gone and toggles the window; NS.COMMANDS 22 -> 23; README *Reporting a bug* (debug on + reproduce, /am diagnostics, /am debug + Copy the whole output into the GitHub issue); counts, slash table row 21, debug.md, smoke 206-209, test-cases (1474), spec DG-1 amendment note |
...
| 2026-09-25 | M4 | tests 1470 passed / 0 failed / 0 skipped (16 shards); luacheck 0 warnings / 0 errors in 130 files; lizard 3863 functions, 0 above CCN 15; head 788e47d |
| 2026-09-25 | P13 | tests 1474 passed / 0 failed / 0 skipped (16 shards); luacheck 0 warnings / 0 errors in 130 files; lizard 3867 functions, 0 above CCN 15; head cf82bd9 |
