...
| P6 empty-container overlap | EO-1 EO-2 | AM | P5 | done | this commit | Anchors.HangMode preview/slot/engine (ApplyVisibility records hangMode); unlocked followers hang from the parent anchor; PlaceAttached memo on mode + strip room; EO-2 as shortfall-only room past a follower parent's beside strip (seam unchanged otherwise, SS-3); test_anchors_hang suite; smoke 191-196 |
...
| P9 re-vendor v1.59.0 + X button | CX-3 | AM | P8, P7 | todo | | |
