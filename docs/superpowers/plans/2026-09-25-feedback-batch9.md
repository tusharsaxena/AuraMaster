...
| Q5 empty-only placeholder (was: drop the slot hang) | HG-1 E1 | Q4 | in progress | | owner amendment 2026-09-25: E1/HG-1 rewritten to keep `slot` gated on `Container:PredictEmpty() == true` (nil and false hang from the engine); the adopted design is appended to the findings as its final section |
