...
| `F-7` | Every row's tooltip is rewritten for two states. **Revised 2026-09-15:** this row originally read *"Hide — never drawn in this container. Show — drawn, unless something else hides it"*, which the priority revision made FALSE — under rank 3 a Show rescues an aura from another category's Hide, so Hide is not "never drawn" and Show is not merely "unless something hides it". The tooltip is row-scoped and says what THIS row does; the full five-rank order lives in the priority blurb (`P-1`), which is the one place it is stated. The old Default/Whitelist/Blacklist wording still goes, from `settings/Filters.lua`, `locales/enUS.lua` and the docs |
