...
| # | Decision |
|---|---|
| E1 | **Unlocked placement always follows the real block.** The `slot` hang mode (B8-P6, EO-1) is removed. While unlocked, a follower hangs from the same rect as when locked: the engine, or the preview extent in test mode. EO-1 is withdrawn. While unlocked, an empty chain collapses (the #9 look), and this is documented as a known limitation: lay chains out in test mode. |
...
| ID | Requirement |
|---|---|
| `HG-1` | `hangModeFor` loses `slot`. Unlocked-and-not-previewing hangs from the engine, as locked does. Remove the slot code path and its tests. Update known-limitations (#9 in the unlocked state) and smoke checks 191-196. |
