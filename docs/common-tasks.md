...
5. **Structural?** If the row changes which rows other pages offer, give it
   ...
   the structure key (`FilterCompiler.StructureKey`) in `Container:Apply` (`modules/Container.lua:359`).
...
2. Append `{ to = 5, apply = function(db) … end }` (the next version) to `SCHEMA_STEPS` in
   `core/Database.lua:727`. The ladder is account-wide (`global.schemaVersion`), but containers live
