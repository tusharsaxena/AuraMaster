...
| `applyContainer` | `applyPass` | `modules/Container.lua:346-378` | One container: compile, place, build or update the engine, restyle, visibility. The call site passes `"applyPass"`, so the record carries observed containment |
...
| `styleElement` | — | `modules/Style.lua:483-491` | Dressing one bar, icon or line of text: called by the engine's `initializeFrame` as it creates buttons, by a restyle, and by the preview |
