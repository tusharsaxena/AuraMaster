...
| `applyContainer` | `applyPass` | `modules/Container.lua:344-387` | One container: compile, place, build or update the engine, restyle, visibility. The call site passes `"applyPass"`, so the record carries observed containment |
...
| `styleElement` | — | `modules/Style.lua:559-567` | Dressing one bar, icon or line of text: called by the engine's `initializeFrame` as it creates buttons, by a restyle, and by the preview |
