...
| `applyContainer` | `applyPass` | `modules/Container.lua:348-380` | One container: compile, place, build or update the engine, restyle, visibility. The call site passes `"applyPass"`, so the record carries observed containment |
...
| `styleElement` | — | `modules/Style.lua:558-566` | Dressing one bar, icon or line of text: called by the engine's `initializeFrame` as it creates buttons, by a restyle, and by the preview |
