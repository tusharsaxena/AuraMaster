...
| `applyContainer` | `applyPass` | `modules/Container.lua:351-394` | One container: compile, place, build or update the engine, restyle, visibility. The call site passes `"applyPass"`, so the record carries observed containment |
...
| `styleElement` | — | `modules/Style.lua:576-584` | Dressing one bar, icon or line of text: called by the engine's `initializeFrame` as it creates buttons, by a restyle, and by the preview |
