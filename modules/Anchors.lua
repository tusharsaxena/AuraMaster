...
--
-- How a follower joins its parent (the two points, their classification into the nine sides, the
-- seam gap, the steady join and the engine lead) is modules/Anchors_Attach.lua, peeled out of this
-- file (AM-ATS-04). It loads first, and this file binds what it needs of it below.
...
-- From modules/Anchors_Attach.lua, at file load: the token -> { side, align } table, a container's
-- own scale, and the points and offsets a container attaches with (attachSpec).
local AA = NS.AnchorsAttach
local EDGE_PARTS, ownScale, attachSpec = AA.EDGE_PARTS, AA.ownScale, AA.Spec
...
-- modules/Anchors_Attach.lua's seamRoom there.
...
-- modules/Anchors_Attach.lua's seamRoom reads both at call time.
AA.furnitureRoom, AA.sideRoom = furnitureRoom, sideRoom
