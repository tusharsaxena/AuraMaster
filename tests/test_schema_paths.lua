...
test("schema paths: ApplyDefault restores a session row through its own set", function()
    ...
    NS2.ApplyDefault(NS2.FindSchemaRow("state.debugConsole"))
...
test("schema paths: a session row's validate still guards it", function()
    ...
    assertTrue(NS2.DebugLog:IsShown())
    local row = NS2.FindSchemaRow("state.debugConsole")
    row.validate = function(v) return v == false end
    local ok, err = NS2.SetByPath("state.debugConsole", "yes")
    assertFalse(ok)
    assertEqual(err, "Invalid value for state.debugConsole")
    assertTrue(NS2.DebugLog:IsShown(), "the refused value never reached set()")
end)
