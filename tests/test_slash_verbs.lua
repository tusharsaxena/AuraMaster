...
local MISSING_ROW = "No container exists yet — create one on the Containers page."
...
test("slash verbs: /am new reads its words in any case, and a later word overrides an earlier one", function()
    local NS2, mocks = fresh()
    local lines = capture(mocks)
    local p = slash(NS2, lines, "NEW Focus Enchants Bar")
    ...
    assertEqual(c.auraType, "ENCHANT")
