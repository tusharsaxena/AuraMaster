...
-- the parent's rect starts at its start corner (0) and is 1 unit across while its engine is empty,
-- one element across while it holds auras or hangs from its anchor or preview. The two must agree.
...
--- Where on screen a join lands on one axis: the relative point on a parent rect `size` units across
--- starting at 0 and running toward `forward` (+1 or -1), at parent scale `ps`, plus the offset `off`
--- at child scale `cs`. `low`/`high` name the axis's sides (LEFT/RIGHT, BOTTOM/TOP).
local function land(side, low, high, size, forward, off, ps, cs)
    local a, b = 0, size * forward
...
local function landX(p, size, growH, ps, cs)
    return land(H_OF[p[3]], "LEFT", "RIGHT", size, growH == "left" and -1 or 1, p[4], ps or 1, cs or 1)
end

local function landY(p, size, growV, ps, cs)
    return land(V_OF[p[3]], "BOTTOM", "TOP", size, growV == "up" and 1 or -1, p[5], ps or 1, cs or 1)
end
...
test("steady: the owner's centered chain growing up lands on the same x with its parent's engine empty as populated", function()
    ...
    near(landX(p, 1, "right"), landX(p, w, "right"), "empty vs populated")
...
test("steady: an end join (right) holds too, and growth left mirrors both", function()
    for _, g in ipairs({ "right", "left" }) do
        for _, pair in ipairs({ { "BOTTOM", "TOP" }, { "BOTTOMRIGHT", "TOPRIGHT" }, { "BOTTOMLEFT", "TOPLEFT" } }) do
            ...
            local p = placed(NS, 3)
            local what = g .. " " .. pair[2]
            near(landX(p, 1, g), landX(p, w, g), what)
...
test("steady: hanging from the parent's one-element anchor (slot) or its preview gives the same place", function()
    ...
    near(landX(slot, w, "right"), landX(engine, 1, "right"), "slot vs empty engine")
    assertEqual(slot[3], engine[3]); assertEqual(slot[4], engine[4])
end)
...
test("steady: a parent one row across (icons filling a row) is steady on y for a side join centered", function()
    ...
    local p = placed(NS, 3)
    near(landY(p, 1, "down"), landY(p, h, "down"), "empty vs populated on y")
...
test("steady: a parent more than one element across is not rewritten on that axis", function()
    ...
    assertEqual(p[3], "TOP"); assertEqual(p[4], 0)
end)
...
test("steady: parent and child at different scales convert the offset to the child's scale", function()
    ...
    local w = NS.Style.ElementSize(NS.Database.FindContainer(2))
    local p = placed(NS, 3)
    near(landX(p, 1, "right", 2, 0.5), landX(p, w, "right", 2, 0.5), "empty vs populated across scales")
    near(p[4], w / 2 * 2 / 0.5, "half the parent's bar, in the child's units")
end)
...
test("steady: start-aligned pairs are placed as before, along the chain and across it", function()
    ...
    local p = placed(NS, 3)
    assertEqual(p[3], "BOTTOMLEFT"); assertEqual(p[4], 0)
