...
- **World of Warcraft (Retail).** Single `## Interface: 120100` line in `AuraMaster.toc:1` — Retail
  ...
  without it, `CM.Init` prints a one-line notice (`modules/ContainerManager.lua:662`) and draws
  nothing.
...
reads the tag named in root `CLAUDE.md` (`v1.59.0`) out of a checkout at `../LibKa0s` and compares
...
git -C ../LibKa0s rev-parse --short v1.59.0   # verify: prints a commit
```
